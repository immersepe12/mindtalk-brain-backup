...
| 15 NEW quarantined pos-100 URLs (06-19 set: art-therapy, abandonment-issues, cbt-for-insomnia, eustress-distress, anxiety-coping-kids, anxious-attachment, hsdd, mindfulness-stress, ptsd-treatment-and-recovery, teen-depression, fear-ending-relationship, double-empathy-autism, panic-attack-calming, psychologists-in-bangalore, therapists-in-hyderabad) | Sat/Mon 2026-06-20/22 next daily rank pull re-checks them | If the *same* ≥4 URLs report exactly 100 AGAIN → escalate to genuine deindex investigation. If recovered (expected, per AP5/P5) → confirmed noise, no watch needed. Assume noise until next pull confirms. |
...
| W11 homepage (check due 2026-06-19 — **DUE TODAY**) | Mixpanel access is BLOCKED (`memory/mixpanel-access-blocked.md`) | 06-19 conversion read is **partial — rank/impr proxy only** (no homepage rank/impr regression in today's signals). Full "Get the App = #1 hero CTA" verdict **deferred** until Mixpanel MCP restored; watch stays open, re-evaluate once T15/T19 conversion data flows. |
